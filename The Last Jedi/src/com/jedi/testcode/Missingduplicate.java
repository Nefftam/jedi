...
public class Missingduplicate {
    public static void main(String[] args) {
        ...
        //A.add(1);A.add(2);A.add(3);A.add(3);A.add(5);
        int len = A.size();

        int sumOriginal = len*(len+1)/2;
        BigDecimal productOriginal = new BigDecimal(1);
        for(int i = 1 ; i <=len ; i++)
        {
            productOriginal = productOriginal.multiply(new BigDecimal(i));
        }
        ...
        {
            sum+=A.get(i);
            product = product.multiply(new BigDecimal(A.get(i)));
        }
        System.out.println(productOriginal);
        System.out.println(product);
        int k = sumOriginal - sum;
        BigDecimal r = productOriginal.divide(product, RoundingMode.HALF_UP);
        BigDecimal missing = r.multiply(new BigDecimal(k)).divide(r.subtract(new BigDecimal(1)));
        BigDecimal duplicate = missing.subtract(new BigDecimal(k));
        ArrayList<Integer> al = new ArrayList<>();
        al.add(duplicate.toBigInteger().intValue());
        al.add(missing.toBigInteger().intValue());
